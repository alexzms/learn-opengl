...
// in blinn-phong, we use half-way direction
...
vec3 calc_point_light(Light light, vec3 normal, vec3 frag_pos, vec3 view_dir)
{
    ...
    // light_dir is object->light_source, view_dir is object->eye, so it's correct
...
//    vec3 reflect_dir = reflect(-light_dir, normal);
    ...
    vec3 specular = pow(max(dot(normal, half_dir), 0.0), material.shininess)
